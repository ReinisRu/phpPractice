...
const TaskList = function (container, callback) {
  ...
  this.addTask = function (text, status) {
    ...
    task.querySelector(".save").addEventListener(
      "click",
      saveChanges.bind({
        element: task,
        tasks_object: this.tasks,
  ...
  const saveChanges = function (event) {
    event.stopPropagation();
    let order = this.element.getAttribute("data-order");
    this.tasks_object[order].text = this.element.querySelector(
      "pre"
    ).textContent;
    ...
    if (result.status === "success") {
      ...
      if (data !== result.data) {
        ...
        for (let i = 0; i < tasks.length; i++) {
          if (
            todo_elements[i] === undefined ||
            !todo_elements[i].classList.contains("editable")
          ) {
